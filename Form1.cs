...
namespace CSharpWinFormProjects
{
    public partial class Form1 : Form
    {
        ...
        double[] oil = { 51.00, 52.00, 55.00, 28.53, 57.00 };
        int tempTimer = 0;
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (PayTotal == 0)
            {
                MessageBox.Show($"", "Завершити роботу", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show($"Продажі за {DateTime.Now.ToShortDateString()} = {Math.Round(PayTotal, 2)} грн.", "Завершити роботу", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            if (tempTimer == 10)
            {
                DialogResult result = MessageBox.Show("Завершити роботу з даним клієнтом?", "Очистити форму?", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes)
                {
                    PayTotal += PayGasStation + PayCafe;
                    timer.Stop();
                    Text = $"BestOil Усі продажі за {DateTime.Now.ToShortDateString()} = {Math.Round(PayTotal, 2)} грн.";
        ...
        private void ToCount_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Ви не вибрали товари!", "Увага!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void Radio_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                text.Text = "0,00";
                MessageBox.Show("Невірні дані!", "Увага!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
